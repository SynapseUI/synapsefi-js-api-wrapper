...
describe('GET_ALL_USER_TRANSACTIONS', () => {
  it('base', async () => {
    try {
      const { data: { trans_count } } = await platformUserApiCannon.GET_ALL_USER_TRANSACTIONS();
  ...
  it('page, per_page', async () => {
    try {
      const { data: { limit, page } } = await platformUserApiCannon.GET_ALL_USER_TRANSACTIONS({
        page: 2,
        per_page: 1,
      });
